...
| 実装済みに見えるサブシステム | 実態 |
|---|---|
| ~~`:TRIGger`~~ | **M5で解消**(1.2)。標準の16種(非シリアル11 + シリアル5)を開放した。当初の内訳: トリガ種別**20種中EDGEの1種のみ**。PULSe / TIMeout / DURation / RUNT / SHOLd / PATTern / DELay / NEDGe / WINDows / SLOPe / VIDeo と、シリアルバストリガ8種(RS232/IIC/SPI/CAN/LIN/FLEXray/IIS/M1553)が全て未実装 |
...
**実装・実機検証とも完了**(2026-08-29。登録Tool 38 → 40。詳細は [tools.md](tools.md) 14章、実測は [verification/mho98-m4-m5.md](verification/mho98-m4-m5.md))。実測で**実装バグ2件**(しきい値の小数形が黙って壊れる / `:MEASure:AMP:TYPE` は短形のみ)と**機器の限界1件**(同時測定項目は12以下)が判明し、いずれも反映済み。
...
**標準搭載の16種を実装済み**(非シリアル11種 + シリアルバス5種)。`configure_trigger` の `type` + `settings` で、新Toolは増やしていない(詳細は [tools.md](tools.md) 15章)。**16種すべてを実機で往復検証済み**(2026-08-29。[verification/mho98-m4-m5.md](verification/mho98-m4-m5.md))。実測で**実装バグ3件**(pattern / duration の ch別リスト、LIN の DATA が2進マスク)が判明し反映済み。

オプション必須の3種(FlexRay / I2S / MIL-STD-1553)と Video は対象外(2章 / 3章)。
...
| **シリアルバス5種** | `:TRIGger:RS232` / `:IIC` / `:SPI` / `:CAN` / `:LIN` | エラーフレーム、特定アドレス/データ。**既存のデコード実装(`configure_decode`)と対になる**(実装済み) |
...
| ~~V-5~~ | `:MEASure:STATistic:ITEM?` の応答形式 | M4 | **解決済み**(2026-08-29)。ガイドどおり科学表記の単一値で、区切り文字は存在しない。実機で裏取り済み |
...
| ~~V-9~~ | `:TRIGger:WINDows:SLOPe` の両エッジの綴り | M5 | **解決済み**(2026-08-29)。ガイドの Range欄 `RFALI` は誤植で実機は `-222` で拒否。Remarks欄の `RFALl` が正しく、`either` として宣言済み |
