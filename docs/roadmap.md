...
### 1.2 M5 — トリガ種別の開放(実装完了・実機検証は一部)

**標準搭載の16種を実装済み**(非シリアル11種 + シリアルバス5種)。`configure_trigger` の `type` + `settings` で、新Toolは増やしていない(詳細は [tools.md](tools.md) 15章)。**実機検証は edge / pulse / i2c の3種のみ実施済み**(2026-08-29。[verification/mho98-m4-m5.md](verification/mho98-m4-m5.md))。残り13種の値域は未検証。
