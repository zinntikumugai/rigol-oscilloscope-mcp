...
| [device-profiles.md](device-profiles.md) | 機種プロファイル仕様。4層解決(verified / family / guide / generic)と、検証済みMHO98プロファイル(quirk集) | 規範(詳細) |
...
| [roadmap.md](roadmap.md) | 今後の対応予定。**AI調査効率で並べた優先度順**(1章=高 / 2章=中 / 3章=低 / 4章=対象外 / 5章=機種プロファイル / 6章=既知の残件 / 7章=検討事項)。完了済みの経緯は末尾にコメントアウトで保存 | 予定・記録 |
...
| [verification/mho98-m4-m5.md](verification/mho98-m4-m5.md) | 測定の全面開放(M4)/ トリガ種別の開放(M5)の実機検証。しきい値の整数形・AMP:TYPEの短形・同時測定項目数の限界を記録 | エビデンス |
